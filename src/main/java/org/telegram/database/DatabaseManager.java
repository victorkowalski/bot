...
public class DatabaseManager {
    ...
    private int createNewTables() throws SQLException {
        ...
        connetion.executeQuery(CreationStrings.createPersonalDataTable);
        connetion.executeQuery(CreationStrings.createUserLanguageDatabase);
        /*
    ...
    //*******************************************************************// Personal Data //*****************************************************
    public int getPersonalDataStatus(Integer userId) {
        int status = -1;
        try {
            final PreparedStatement preparedStatement = connetion.getPreparedStatement("SELECT status FROM personal_data WHERE user_id = ?");
            preparedStatement.setInt(1, userId);
            final ResultSet result = preparedStatement.executeQuery();
            if (result.next()) {
                status = result.getInt("status");
    ...
    public boolean addFirstName(Integer userId, int status, int messageId, String firstName) {
        int updatedRows = 0;
        String updateSql = "UPDATE personal_data SET status = ?, message_id = ?, first_name = ?"
                + " WHERE user_id = ?";
        try {
            final PreparedStatement preparedStatement = connetion.getPreparedStatement(updateSql);
    ...
    public boolean addLastName(Integer userId, int status, int messageId, String lastName) {
        int updatedRows = 0;
        String updateSql = "UPDATE personal_data SET status = ?, message_id = ?, last_name = ?"
                + " WHERE user_id = ?";
        try {
            final PreparedStatement preparedStatement = connetion.getPreparedStatement(updateSql);
            preparedStatement.setInt(1, status);
    ...
    public boolean addPhone(Integer userId, int status, int messageId, String phone) {
        int updatedRows = 0;
        String updateSql = "UPDATE personal_data SET status = ?, message_id = ?, phone = ?"
                + " WHERE user_id = ?";
        try {
            final PreparedStatement preparedStatement = connetion.getPreparedStatement(updateSql);
            preparedStatement.setInt(1, status);
    ...
    public boolean waitFirstName(Integer userId, int status, int messageId) {
        int updatedRows = 0;
        String insertSql = "INSERT INTO personal_data (user_id, status, message_id) VALUES (?, ?, ?)";

        try {
            final PreparedStatement preparedStatement = connetion.getPreparedStatement(insertSql);
            preparedStatement.setInt(1, userId);
            preparedStatement.setInt(2, status);
            preparedStatement.setInt(3, messageId);
    ...
    public int getPersonalDataMessageId(Integer userId) {
        int messageId = 0;
        try {
            final PreparedStatement preparedStatement = connetion.getPreparedStatement("SELECT message_id FROM personal_data WHERE user_id = ?");
            preparedStatement.setInt(1, userId);
            final ResultSet result = preparedStatement.executeQuery();
            if (result.next()) {
